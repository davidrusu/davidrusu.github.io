...
Elm.TeaLog = Elm.TeaLog || {};
Elm.TeaLog.make = function (_elm) {
   "use strict";
   _elm.TeaLog = _elm.TeaLog || {};
   if (_elm.TeaLog.values)
   return _elm.TeaLog.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $Css$Float = Elm.Css.Float.make(_elm),
   ...
   var viewVender = F2(function (address,
   vender) {
      return function () {
         var url = function () {
            switch (vender.ctor)
            {case "DistinctlyTea":
               return "http://distinctlytea.com/";
               case "YunnanSourcing":
               return "https://yunnansourcing.com/";}
            _U.badCase($moduleName,
            "between lines 175 and 178");
         }();
         return A2($Html.div,
         _L.fromArray([]),
         _L.fromArray([$Html.text("I got this tea from ")
                      ,A2($Html.a,
                      _L.fromArray([$Html$Attributes.href(url)]),
                      _L.fromArray([$Html.text($Basics.toString(vender))]))]));
      }();
   });
   var viewStar = A2($Html.div,
   ...
   var viewRating = F2(function (address,
   rating) {
      return $Html.div(_L.fromArray([]))(A2($List.repeat,
      rating,
      viewStar));
   });
   var inline = $Css$Display.display($Css$Display.InlineBlock);
   var viewRegion = function (region) {
      return function () {
         switch (region.ctor)
         {case "UnknownRegion":
            return ", I don\'t know which region this tea is from";}
         return A2($Basics._op["++"],
         " from the ",
         A2($Basics._op["++"],
         $Basics.toString(region),
         " region"));
      }();
   };
   ...
   var viewTeaCategory = function (category) {
      return function () {
         switch (category.ctor)
         {case "Blend":
            return function () {
                 var lastPart = function () {
                    var _v7 = last(category._0);
                    switch (_v7.ctor)
                    {case "Just":
                       return A2($Basics._op["++"],
                         " and ",
                         viewTeaCategory(_v7._0));
                       case "Nothing": return "";}
                    _U.badCase($moduleName,
                    "between lines 148 and 151");
                 }();
                 var listPart = $String.join(", ")($List.map(viewTeaCategory)(A2($List.take,
                 $List.length(category._0) - 1,
                 category._0)));
                 return A2($Basics._op["++"],
                 "Blend of ",
                 A2($Basics._op["++"],
         ...
         return $Basics.toString(category);
      }();
   };
   var viewTea = F2(function (address,
   tea) {
      return function () {
         var teaName = function () {
            var _v9 = tea.url;
            switch (_v9.ctor)
            {case "Just": return A2($Html.a,
                 _L.fromArray([$Html$Attributes.href(_v9._0)]),
                 _L.fromArray([$Html.text(tea.name)]));
               case "Nothing":
               return $Html.text(tea.name);}
            _U.badCase($moduleName,
         ...
         _L.fromArray([A2($Html.u,
                      _L.fromArray([]),
                      _L.fromArray([teaName]))
                      ,A2($Html.br,
                      _L.fromArray([]),
                      _L.fromArray([]))
                      ,$Html.text(A2($Basics._op["++"],
                      viewTeaCategory(tea.category),
                      A2($Basics._op["++"],
                      " tea ",
                      viewRegion(tea.region))))]));
      }();
   });
   var viewLogEntry = F2(function (address,
   log) {
      return function () {
         var styles = A4($Css$Margin.all,
         10,
         10,
         10,
         10)(A4($Css$Padding.all,
         10,
         0,
         10,
         0)($Css$Font.size(12)(_L.fromArray([{ctor: "_Tuple2"
                                             ,_0: "borderTop"
                                             ,_1: "1px solid black"}]))));
         return A2($Html.div,
         _L.fromArray([$Html$Attributes.style(styles)]),
         _L.fromArray([A2($Html.div,
                      _L.fromArray([$Html$Attributes.style($Css$Border$Bottom.color($Color.black)($Css$Border$Bottom.width(1)(_L.fromArray([]))))]),
                      _L.fromArray([A2($Html.div,
                                   _L.fromArray([$Html$Attributes.style(inline(_L.fromArray([])))]),
                                   _L.fromArray([$Html.text(log.date)]))
                                   ,A2($Html.div,
                                   _L.fromArray([$Html$Attributes.style($Css$Float.$float($Css$Float.Right)(_L.fromArray([])))]),
                                   _L.fromArray([A2(viewRating,
                                   address,
                                   log.rating)]))]))
                      ,A2(viewTea,address,log.tea)
                      ,$Markdown.toHtml(log.review)
                      ,A2(viewVender,
                      address,
                      log.vendor)]));
      }();
   ...
   var centered = function (styles) {
      return $Css$Flex.alignItems($Css$Flex.AICenter)($Css$Flex.justifyContent($Css$Flex.JCCenter)($Css$Display.display($Css$Display.Flex)(styles)));
   };
   var update = F2(function (action,
   model) {
      return function () {
   ...
   model) {
      return function () {
         var styles = centered(_L.fromArray([]));
         return A2($Html.div,
         _L.fromArray([]),
         _L.fromArray([$Utils.css("../style.css")
                      ,A2($Header.view,
                      A2($Signal.forwardTo,
                      address,
                      ModifyHeader),
   ...
   var YunnanSourcing = {ctor: "YunnanSourcing"};
   var DistinctlyTea = {ctor: "DistinctlyTea"};
   var Tea = F7(function (a,
   b,
   c,
   d,
   e,
   ...
   var Blend = function (a) {
      return {ctor: "Blend",_0: a};
   };
   var Herbal = {ctor: "Herbal"};
   var Darjeeling = {ctor: "Darjeeling"};
   var PuErh = function (a) {
      return {ctor: "PuErh",_0: a};
   };
   var Black = {ctor: "Black"};
   var Oolong = {ctor: "Oolong"};
   var Green = {ctor: "Green"};
   var Yellow = {ctor: "Yellow"};
   var White = {ctor: "White"};
   var AgedRaw = {ctor: "AgedRaw"};
   var Ripened = {ctor: "Ripened"};
   var Raw = {ctor: "Raw"};
   var Maocha = {ctor: "Maocha"};
   var UnknownRegion = {ctor: "UnknownRegion"};
   var Nepal = {ctor: "Nepal"};
   var Java = {ctor: "Java"};
   var Kenya = {ctor: "Kenya"};
   var Sumatra = {ctor: "Sumatra"};
   var Ceylon = {ctor: "Ceylon"};
   var Nilgiri = {ctor: "Nilgiri"};
   var DarjeelingDistrict = {ctor: "DarjeelingDistrict"};
   var Assam = {ctor: "Assam"};
   var LapsangSouchong = {ctor: "LapsangSouchong"};
   var Yunnan = {ctor: "Yunnan"};
   var Keemum = {ctor: "Keemum"};
   var UnknownSeason = {ctor: "UnknownSeason"};
   var Winter = {ctor: "Winter"};
   var Autumn = {ctor: "Autumn"};
   var Summer = {ctor: "Summer"};
   var Spring = {ctor: "Spring"};
   var Pineapple = {ctor: "Pineapple"};
   var Curry = {ctor: "Curry"};
   var Corn = {ctor: "Corn"};
   var Grenadine = {ctor: "Grenadine"};
   var Vanilla = {ctor: "Vanilla"};
   var log = _L.fromArray([{_: {}
                           ,date: "2015-11-19"
                           ,rating: 9
                           ...
                           ,tea: {_: {}
                                 ,category: Black
                                 ,flavours: _L.fromArray([])
                                 ,name: "Wild Tree Purple Varietal Black Tea of Dehong"
                                 ,region: Yunnan
                                 ,season: Spring
                                 ,url: $Maybe.Just("http://yunnansourcing.com/en/yunnan-black-teas/2384-light-roast-wild-tree-purple-varietal-black-tea-of-dehong-spring-2013.html")
                                 ,year: $Maybe.Just(2013)}
                          ...
                          ,{_: {}
                           ...
                           ,tea: {_: {}
                                 ,category: Blend(_L.fromArray([White
                                                               ,Green
                                                               ,Darjeeling
                                                               ,Herbal]))
                                 ,flavours: _L.fromArray([Corn
                                                         ,Curry
                                                         ,Pineapple])
                                 ,name: "Himalayan"
